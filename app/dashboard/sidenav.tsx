...
export default function SideNav() {
  ...
  return (
    <div className={`flex h-full flex-col text-white sidebar ${sidebarMinimized ? 'minimized' : ''}`}>
  <div className="flex items-center justify-start space-x-6 bg-black p-4 md:justify-left">
    <div
      className={`flex items-center space-x-6 cursor-pointer sidebar-header ${sidebarMinimized ? 'rounded-tr-full rounded-br-full' : ''}`}
      onClick={toggleSidebar}
    >
      <HamburgerMenuIcon className={`w-6 h-6 text-white ${sidebarMinimized ? 'rotate-90' : ''}`} />
